...
def launchTool():
	...
	cmds.separator( height=10, style='in' )
	cmds.button( label='Disable Motion Blur', command='cmds.setAttr("miDefaultOptions.motionBlur",0)')
	cmds.button( label='Enable Motion Blur No Def', command='cmds.setAttr("miDefaultOptions.motionBlur",1)')
	cmds.button( label='Load Preview Preset', command='mel.eval("""loadNodePresets \"Preview\";""")')
	cmds.button( label='Load Production Preset', command='mel.eval("""loadNodePresets \"Production\";""")')
	cmds.separator( height=10, style='in' )
	cmds.button( label='Start Batch Render', command='mel.eval("BatchRender;")')
	cmds.button( label='Cancel Batch Render', command='mel.eval("CancelBatchRender;")')
	print myWindow
